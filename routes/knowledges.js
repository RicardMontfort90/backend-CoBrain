...
router.get('/:knowledgeId', isAuthenticated, async (req, res, next) => {
  ...
  // @access  Private
router.post('/', isAuthenticated,  async (req, res, next) => { // IS AUTHENTICATED
    const { category, title, knowledgeImage, timeOfActivity, location, description } = req.body;
    // if (!category || !title ||  !knowledgeImage, !timeOfActivity, !description)
    // res.status(400).json({message: "Please check your fields"})
    const userId = req.payload._id;
    try {
        const newKnowledge = await Knowledge.create({ userId: userId, category, title, knowledgeImage, timeOfActivity, location, description }); // INSTEAD OF PASSING BODY GLOBALLY, DESTRUCTURE, CHECK ALL EXPECTED FIELDS ARE PRESENT, IF NOT RES.STATUS AND ERROR.
...
router.put('/:knowledgeId', isAuthenticated,  async (req, res, next) => { // IS AUTHENTICATED
    ...
    try {
        ...
        const editKnowledge = await Knowledge.findByIdAndUpdate(knowledgeId, { category, title, knowledgeImage, timeOfActivity, location, description }, { new: true }); // INSTEAD OF PASSING BODY GLOBALLY, DESTRUCTURE, CHECK ALL EXPECTED FIELDS ARE PRESENT, IF NOT RES.STATUS AND ERROR.
  ...
  // @access  Private
router.delete('/:knowledgeId', isAuthenticated,async (req, res, next) => { // IS AUTHENTICATED
    const { knowledgeId } = req.params;
    // IF (KNOWLEDGE.USER !== REQ.PAYLOAD._ID) RETORNAR ERROR "UNAUTHORIZED TO EDIT"
    try {
        const knowledge = await Knowledge.findById(knowledgeId);
        ...
        res.status(200).json({deletedKnowledge, message: "OK"}); // NOT NEEDED TO SEND DELETED KNOWLEDGE, JUST "OK"
    } catch (error) {
        next(error)
    }
});
